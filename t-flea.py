...
def get_username():
    if make_arguments().username:
        username = make_arguments().username
    else:
...
def make_arguments():
    parser = argparse.ArgumentParser(description="Bulk downloads up to 200 of "
                                     ...
                                     "Lichess.org")
    parser.add_argument("-u", "--username", help="Lichess.org username")
...
def choose_target_dir():
    if make_arguments().directory:
        ...
        target_dir = "lichess_pgns/" + get_username() + "/"
    return target_dir
...
config.read("/home/kevin/Development/Python/app.config")

games_data = fetch_games_data(get_username(), make_arguments().num)
